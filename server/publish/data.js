...
Meteor.publish('core_data', function(SeasonId) {
	var athleteQuery = SeasonId ? {activeSeasons: SeasonId} : {active: true};
	return [
		...
		Athletes.find(athleteQuery, {fields: {FamilyName: 1, GenderId: 1, GivenName: 1, IBUId: 1, NAT: 1, activeSeasons: 1, value: 1, points: 1}}),
		Nations.find({})
	];
});
...
Meteor.publish('results', function(id, season) {
	return Results.find({$or: [{_id: id, SeasonId: season}, {IBUId: id, SeasonId: season}]});	
});
